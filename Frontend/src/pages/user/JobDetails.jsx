...
function JobDetails() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:4000/api/jobs/apply/${id}`,
        {
          method: "POST",
          credentials: "include",
  ...
  if (!job)
    return <p style={{ padding: "40px", color: "#111" }}>Loading...</p>;
  ...
  return (
    <div style={container}>
      ...
      <div style={left}>
        ...
        <p style={salary}>
          <b>Salary:</b> ₹{job.salary || "Not disclosed"}
        </p>

        {job.deadline && (
          <p style={deadlineStyle(isDeadlinePassed)}>
            <b>Application Deadline:</b> {new Date(job.deadline).toLocaleDateString(undefined, {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
      ...
      <div style={right}>
        ...
        {!isDeadlinePassed && !status && (
          <div style={uploadBox}>
            <label style={uploadLabel} htmlFor="resume-upload">
              📎 Resume (PDF, optional)
            </label>
            <input
              id="resume-upload"
            ...
            />
            {resumeFile && (
              <p style={fileNameText}>✅ {resumeFile.name}</p>
            )}
          </div>
        )}
